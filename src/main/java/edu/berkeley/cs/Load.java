...
public class Load {
  ...
  public static void main(String[] args) throws ConfigurationException, IOException {
    Configuration config =
      new PropertiesConfiguration(Load.class.getResource("/benchmark.properties"));
    Configuration titanConfiguration =
      new PropertiesConfiguration(Load.class.getResource("/titan-cassandra.properties"));
  ...
  private static void loadGraph(TitanGraph g, Configuration conf) throws IOException {
    ...
    System.out
      .printf("nodeFile %s, edgeFile %s, propertySize %d\n", nodeFile, edgeFile, propertySize);
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(nodeFile))) {
      for (String line; (line = br.readLine()) != null; ) {
        ...
        if (++c % 1000L == 0L) {
          ...
          System.out.println(
            "Processed " + c + " nodes in " + (now - startTime) + "ms (" + (c * 1000.0 / (now
              - startTime)) + " nodes/s)");
          bg.commit();
        }
      }
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(edgeFile))) {
      for (String line; (line = br.readLine()) != null; ) {
        ...
        if (++c % 1000L == 0L) {
          ...
          bg.commit();
          System.out.println(
            "Processed " + c + " edges in " + (now - startTime) + "ms (" + (c * 1000.0 / (now
              - startTime)) + " edges/s)");
        }
      }
    }
