...
public abstract class Benchmark<T> {
  ...
  public static PrintWriter makeFileWriter(String outputName, boolean append) {
    try {
      return new PrintWriter(
        new BufferedWriter(new FileWriter(Paths.get(outputPath, outputName).toFile(), append)));
    } catch (IOException e) {
      e.printStackTrace();
      return null;
    }
  ...
  public void benchLatency() {
    ...
    for (int i = 0; i < MEASURE_N; i++) {
      ...
      long numResults =
        (results instanceof Collection<?>) ? ((Collection<?>) results).size() : (Long) results;
  ...
  public void benchThroughput(int numClients) {
    ...
    System.out
      .println("Titan " + benchClassName + " query throughput with " + numClients + " clients.");
