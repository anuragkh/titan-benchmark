...
public class NeighborNode extends Benchmark<List<Long>> {
  ...
  static void getNeighborNodeQueries(String file, long[] indices, int[] attributes,
    String[] queries) {
  ...
  @Override public void readQueries() {
    getNeighborNodeQueries(WARMUP_FILE, warmupNeighborNodeIds, warmupNeighborNodeAttrIds,
      warmupNeighborNodeAttrs);
    getNeighborNodeQueries(QUERY_FILE, neighborNodeIds, neighborNodeAttrIds, neighborNodeAttrs);
  }

  @Override public List<Long> warmupQuery(Graph g, int i) {
    return g.getNeighborNode(warmupNeighborNodeIds[i], warmupNeighborNodeAttrIds[i],
      warmupNeighborNodeAttrs[i]);
  }

  @Override public List<Long> query(Graph g, int i) {
    return g.getNeighborNode(neighborNodeIds[i], neighborNodeAttrIds[i], neighborNodeAttrs[i]);
  }
