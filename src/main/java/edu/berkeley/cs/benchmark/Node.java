...
public class Node extends Benchmark<Set<Long>> {
  ...
  static void getNodeQueries(String file, int[] indices1, int[] indices2, String[] queries1,
    String[] queries2) {
  ...
  @Override public void readQueries() {
    getNodeQueries(WARMUP_FILE, warmupNodeAttrIds1, warmupNodeAttrIds2, warmupNodeAttrs1,
      warmupNodeAttrs2);
    getNodeQueries(QUERY_FILE, nodeAttrIds1, nodeAttrIds2, nodeAttrs1, nodeAttrs2);
  }

  @Override public Set<Long> warmupQuery(Graph g, int i) {
    return g.getNodes(warmupNodeAttrIds1[i], warmupNodeAttrs1[i]);
  }

  @Override public Set<Long> query(Graph g, int i) {
